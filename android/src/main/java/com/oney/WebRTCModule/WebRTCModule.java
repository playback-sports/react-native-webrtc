...
public class WebRTCModule extends ReactContextBaseJavaModule {
    ...
    private PeerConnection.RTCConfiguration parseRTCConfiguration(ReadableMap map) {
        ...
        if (map.hasKey("sdpSemantics") && map.getType("sdpSemantics") == ReadableType.String) {
            String v = map.getString("sdpSemantics");
            if (v.equals("unified-plan")) {
                conf.sdpSemantics = PeerConnection.SdpSemantics.UNIFIED_PLAN;
            } else if (v.equals("plan-b")) {
    ...
    public void mediaStreamTrackSetVolume(String trackId, double volume) {
        ThreadUtils.runOnExecutor(() -> {
            for (int i = 0, size = mPeerConnectionObservers.size(); i < size; i++) {
                PeerConnectionObserver pco = mPeerConnectionObservers.valueAt(i);
                pco.remoteStreams.values().forEach(stream -> {
